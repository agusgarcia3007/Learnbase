...
export const tenantsRoutes = new Elysia({ prefix: "/tenants", name: "tenants-routes" })
  .use(authPlugin)
  ...
  .post(
    ...
    },
    {
      ...
      detail: { tags: ["Tenants"], summary: "Create a new tenant (superadmin only)" },
    }
  )
  .get("/", async () => {
    ...
    detail: { tags: ["Tenants"], summary: "List all tenants (superadmin only)" },
  })
  .get(
    "/:slug",
    async ({ params, set }) => {
    ...
    {
      detail: { tags: ["Tenants"], summary: "Get tenant by slug (superadmin only)" },
    }
  )
